...
def lambda_handler(event, context):
    principal_id = 'github'
    arn_split = event['methodArn'].split(':')
    aws_account_id = arn_split[4]
    api_gateway_arn_split = arn_split[5].split('/')

    policy = AuthPolicy(principal_id, aws_account_id)
    policy.restApiId = api_gateway_arn_split[0]
    policy.stage = api_gateway_arn_split[1]

    # Only allow GithubType type to be ingested through this endpoint
    policy.allowMethod(HttpVerb.POST, '/' + api_gateway_arn_split[3] + '/GithubType')
    policy.region = arn_split[3]
...
class AuthPolicy(object):
    ...
    def _addMethod(self, effect, verb, resource, conditions):
        ...
        resourceArn = 'arn:aws:execute-api:{}:{}:{}/{}/{}/{}'.format(self.region,
                                                                     self.awsAccountId,
                                                                     self.restApiId, self.stage,
                                                                     verb, resource)
